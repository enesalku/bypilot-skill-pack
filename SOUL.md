...
6. **Self-improving via observation, not introspection** — Hooks log 100% of tool use. A background observer agent clusters patterns into instincts. High-confidence instincts graduate to skills via explicit `/promote` — never silently.
...
The main `/bypilot run` skill is the conductor; it owns no domain logic, only sequencing.
...
/bypilot run      → wave loop until all done or human gate
